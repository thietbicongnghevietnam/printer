...
import 'package:smart_warehouse/views/pages/temporary_area/combine_pallet/move_receiving_card_controller.dart';
...
void main() {
  ...
  group('Update Pallet Source', () {
    test('Update pallet source when pallet valid', () async {
      ...
      // Act
      await controller.updatePalletSource('PL199');
    ...
    test('Update pallet source when pallet invalid', () async {
      ...
      // Act
      await expectLater(
        () async => controller.updatePalletSource('1111'),
    ...
    test('Update pallet source same with pallet end', () async {
      ...
      // Act
      await expectLater(
        () async => controller.updateLocation('PL199'),
        throwsA(isA<ValidationError>()),
      );
  ...
  group('Added Pallet End', () {
    test('Update pallet end when pallet valid', () async {
      ...
      // Act
      await controller.updateLocation('PL199');
    ...
    test('Update pallet source when pallet invalid', () async {
      ...
      // Act
      await expectLater(
        () async => controller.updateLocation('1111'),
        throwsA(isA<ValidationError>()),
      );
