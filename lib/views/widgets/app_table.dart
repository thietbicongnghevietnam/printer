...
typedef AppTableFetch = Future<AppTableResponse>
Function(AppTableRequest);

/// Request data to get data when scrolling has reached the end.
class AppTableRequest {
  AppTableRequest({
    this.lastRow,
    this.sortColumn,
...
class PlutoGridRefetchDataEvent extends PlutoGridEvent {
  @override
...
class AppTable extends StatefulWidget {
  ...
  final AppTableFetch fetch;

  final PlutoGridStateManager stateManager;

  @override
  State<AppTable> createState() =>
      _AppTableState();
}
...
class _AppTableState extends State<AppTable> {
  ...
  void _eventListener(PlutoGridEvent event) {
    if (event is PlutoGridCannotMoveCurrentCellEvent &&
      ...
      _update(null);
    } else if (event is PlutoGridRefetchDataEvent) {
      _update(null);
    }
  }
