...
@immutable
class ZoomMapWidget extends StatefulWidget {
  ZoomMapWidget({
  ...
  static Vector3 getNearestPointOnLine(Vector3 point, Vector3 l1, Vector3 l2) {
    ...
    final l1P = point - l1;
    final l1L2 = l2 - l1;
    final fraction = (l1P.dot(l1L2) / lengthSquared).clamp(0.0, 1.0);
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final closestPoints = <Vector3>[
      ZoomMapWidget.getNearestPointOnLine(point, quad.point0, quad.point1),
      ZoomMapWidget.getNearestPointOnLine(point, quad.point1, quad.point2),
      ZoomMapWidget.getNearestPointOnLine(point, quad.point2, quad.point3),
      ZoomMapWidget.getNearestPointOnLine(point, quad.point3, quad.point0),
    ];
  ...
  @override
  State<ZoomMapWidget> createState() => _ZoomMapWidgetState();
}

class _ZoomMapWidgetState extends State<ZoomMapWidget>
  ...
  Rect get _boundaryRect {
    ...
    final boundaryRect = EdgeInsets.zero.inflateRect(Offset.zero & childSize);
    assert(
  ...
  double _getScrollBarLength(Matrix4 matrix,
    ...
    switch (scrollType) {
      ...
      case _ScrollType.vertical:
        percent =
            parentSize.height / (childSize.height * matrix.getMaxScaleOnAxis());
  ...
  Matrix4 _matrixTranslate(Matrix4 matrix, Offset translation,
    ...
    final alignedTranslation = translation;
  ...
  @override
  void didUpdateWidget(ZoomMapWidget oldWidget) {
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return ZoomMapWidget.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside =
        ZoomMapWidget.getNearestPointInside(point, boundary);
